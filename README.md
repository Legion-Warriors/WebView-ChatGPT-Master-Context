...
- [`CORE_CONTEXT.md`](CORE_CONTEXT.md)
...
- [`CONTEXT_CHECKPOINT.md`](CONTEXT_CHECKPOINT.md)
...
1. Use [`LinkedIn_Post_Hierarchy_Conversation.md`](LinkedIn_Post_Hierarchy_Conversation.md) as the canonical source of truth.
2. Use [`ANDROID_WEBVIEW_SERIES_ARCS.md`](ANDROID_WEBVIEW_SERIES_ARCS.md) as the normalized execution subset of that source.
3. Use [`CORE_CONTEXT.md`](CORE_CONTEXT.md) for stable strategy and constraints.
4. Use [`SERIES_STATE.md`](SERIES_STATE.md) for quick “current status + next action.”
5. Use [`CONTEXT_CHECKPOINT.md`](CONTEXT_CHECKPOINT.md) when a full context restore is needed.
6. Save produced post outputs in [`linkedin_posts/`](linkedin_posts/).

## Maintenance Workflow

When adding a new post artifact:

1. Create/save the post file in [`linkedin_posts/`](linkedin_posts/).
2. Update [`SERIES_STATE.md`](SERIES_STATE.md):
   - Add completion line for the new artifact.
   - Update completed/next status.
3. Mirror the same checkpoint update in [`CONTEXT_CHECKPOINT.md`](CONTEXT_CHECKPOINT.md).
4. If strategy or scope changed, update [`CORE_CONTEXT.md`](CORE_CONTEXT.md).
5. Commit all related files in one commit so state and artifact stay synchronized.

## Naming Conventions

- Post files: [`linkedin_posts/Post <index><suffix>`](linkedin_posts/)
...
- Completed artifact: [`linkedin_posts/Post 1A`](linkedin_posts/Post%201A)
